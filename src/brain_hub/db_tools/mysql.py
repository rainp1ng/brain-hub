...
class RainDB(object):
    ...
    def drop_table(self, table_name):
        ...
        if len(res) == 0:
            return
        d = input("Press y to drop table %s ." % table_name)
        if d == "y":
    ...
    def concate_update(self, val):
        n_update = ""
        for i, desc in enumerate(val):
            n_update += "%s=VALUES(%s)," % (desc, desc)
            if i == len(val)-1:
                n_update = n_update[: -1]
    ...
    def insert_or_update(self, table, val, auto_commit=True):
        '''
        insert data, update if exists
        :param table:
        :param val:
        :param auto_commit:
        :return:
        '''
        n_desc, n_val = self.concat_new_records(val)
        n_update = self.concate_update(val)
        sql_str = "INSERT INTO %s %s VALUES %s ON DUPLICATE KEY UPDATE %s" % (table, n_desc, n_val, n_update)
        self.execute(sql_str)
        if auto_commit:
            self.commit()
    ...
    def batch_insert_or_update(self, table, vals, _async=False, parrallels=1):
        ...
        if _async:
            ...
            for i in range(0, parrallels + 1):
                max = (i + 1) * max_batch_nums if (i + 1) * max_batch_nums < cnt else cnt
                process = multiprocessing.Process(target=self.sync_batch_insert_or_update,
                                                  name=str(i), args=(table, vals[i * max_batch_nums: max]))
                process.start()

        else:
            self.sync_batch_insert_or_update(table, vals)

    def sync_batch_insert_or_update(self, table, vals):
        for n, val in enumerate(vals):
            self.insert_or_update(table, val, False)
            if n % self.batch_num == 0:
                self.commit()
        self.commit()
