...
def get_connection(host, port, user, passwd, database, charset='utf-8'):
    '''
    ...
    '''
    return psycopg2.connect(host=host, user=user, password=passwd, database=database, port=port)
...
class RainDB(object):
    ...
    demo:
        ...
        |   ...
    '''
    def __init__(self, host, port, user, passwd, db_name="", charset="utf-8", encode="utf8mb4", batch_num=1500):
        '''
        ...
        self.db = get_connection(host, port, user, passwd, db_name, charset)
    ...
    def tables(self, schema):
        return self.query("SELECT tablename FROM pg_catalog.pg_tables where schemaname='%s'" % schema)
    ...
    def concat_new_records(self, val):
        ...
        for i, desc in enumerate(val):
            n_desc += desc + ","
            n_val += self.escape_string(str(val[desc]))+"','"
            if i == len(val)-1:
    ...
    def insert(self, table, val, auto_commit=True):
        ...
        sql_str = "insert into %s %s values %s " % (table, n_desc, n_val)
    ...
    def replace(self, table, val, auto_commit=True, sql_template="REPLACE INTO %s %s VALUE %s "):
        '''
        ...
        '''
        n_desc, n_val = self.concat_new_records(val)
        sql_str = sql_template % (table, n_desc, n_val)
        logging.info(sql_str)
        self.execute(sql_str)
        if auto_commit:
            self.commit()

    def batch_replace(self, table, vals, sql_template="REPLACE INTO %s %s VALUE %s "):
        '''
        ...
        '''
        for n, val in enumerate(vals):
            self.replace(table, val, False, sql_template)
            if n % self.batch_num == 0:
                self.commit()
        self.commit()
